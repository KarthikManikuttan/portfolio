...
class MeteorShower extends StatefulWidget {
  ...
  const MeteorShower({
    super.key,
    required this.child,
    this.numberOfMeteors = 100, // Changed to 100
...
class MeteorShowerState extends State<MeteorShower>
  ...
  void _initializeMeteors(Size size) {
    // Fixed: Check if list length matches numberOfMeteors
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Stack(
          children: [
            widget.child,
            ...List.generate(_meteors.length, (index) {
              // Use _meteors.length instead of widget.numberOfMeteors
...
class Meteor {
  ...
  Meteor(double angle, Size size)
      : startX = Random().nextDouble() * size.width, // Full width
        startY =
            -Random().nextDouble() * size.height * 0.3, // Start above screen
        delay = Random().nextDouble(),
        duration = 0.3 + Random().nextDouble() * 0.7 {
    var distance = size.height * 1.2; // Longer distance for full coverage
